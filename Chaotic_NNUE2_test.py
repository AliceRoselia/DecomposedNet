...
class singlePerspectiveNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(1, 4, 3,padding = "same", padding_mode="circular")
        self.conv2 = nn.Conv1d(4, 1, 3,padding = "same", padding_mode="circular")
    ...
    def forward(self,x):
        combined_feature = self.combined_net(x).unsqueeze(-2)
        combined_feature = torch.clamp(combined_feature,0,1)
        combined_feature = self.conv1(combined_feature)
        combined_feature = torch.clamp(combined_feature,0,1)
        combined_feature = self.conv2(combined_feature).squeeze()
...
model.load_state_dict(torch.load("ChaoticNet2_epoch1.pt",weights_only=True))
